...
sealed class MyBookmarkBoardViewEvent {
    data class ChatStart(val chatStartEntity: ChatStartEntity) : MyBookmarkBoardViewEvent()
    data class Error(val errorCode: Throwable) : MyBookmarkBoardViewEvent()
}

class MyBookmarkBoardViewModel @Inject constructor(
    private val loadMyBookmarkBoardListUseCase: LoadMyBookmarkBoardListUseCase,
    ...
    private val _viewEvent = MutableSharedFlow<MyBookmarkBoardViewEvent>()
    val viewEvent: SharedFlow<MyBookmarkBoardViewEvent> = _viewEvent.asSharedFlow()
    ...
    suspend fun loadMyBookmarkBoardList(): MyBoardViewState {
    ...
    suspend fun startChat(
        ...
        kotlin.runCatching {
            ...
            when (chatRoomCheckEntity.isChatRoom) {
                true -> {
                    _viewEvent.emit(
                        MyBookmarkBoardViewEvent.ChatStart(
            ...
            _viewEvent.emit(MyBookmarkBoardViewEvent.Error(it))
        }
    }
